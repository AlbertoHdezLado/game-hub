...
export function DetectiveClubPage() {
  ...
  return <GameThemeProvider theme={game.theme}><main className="game-shell"><header className="game-header"><Link className="icon-button" to="/" aria-label="Volver al hub">⌂</Link><span>{game.title}</span><Link className="icon-button" to="/" aria-label="Ayuda">?</Link></header>
    {phase === 'setup' && <section className="game-panel setup-panel"><span className="eyebrow">Configuración</span><h1>Detective Club</h1><label className="names-field">Jugadores separados por comas<input value={names.join(', ')} onChange={(event) => setNames(event.target.value.split(','))} /></label><label className="field-label">Vueltas <strong>{laps}</strong><input type="range" min="1" max="3" value={laps} onChange={(event) => setLaps(Number(event.target.value))} /></label>{error && <p className="form-message">{error}</p>}<button className="primary-button" type="button" onClick={startMatch}>Iniciar partida</button></section>}
    ...
    {activeReveal !== null && <div className="dialog-backdrop" role="presentation" onClick={closeReveal}><section className="reveal-dialog" role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}><span className="eyebrow">{names[activeReveal]}</span>{!roleVisible ? <><h2>Tu tarjeta está oculta</h2><button className="primary-button" type="button" onClick={() => setRoleVisible(true)}>Revelar</button></> : <><h2>{activeReveal === conspirator ? 'Conspirador' : 'Detective'}</h2><p className="role-word">{activeReveal === conspirator ? 'No conoces la palabra.' : word}</p><button className="primary-button" type="button" onClick={closeReveal}>Ocultar y pasar</button></>}</section></div>}
